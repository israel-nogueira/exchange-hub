...
class OkxSigner
{
    ...
    {
        $ts        = gmdate("Y-m-d\TH:i:s") . '.' . str_pad((string)(microtime(true) * 1000 % 1000), 3, '0') . 'Z';
        $signature = base64_encode(hash_hmac('sha256', $ts . strtoupper($method) . $path . $body, $this->secret, true));
        $headers   = ['OK-ACCESS-KEY'=>$this->apiKey,'OK-ACCESS-SIGN'=>$signature,'OK-ACCESS-TIMESTAMP'=>$ts,'OK-ACCESS-PASSPHRASE'=>$this->passphrase,'Content-Type'=>'application/json'];
