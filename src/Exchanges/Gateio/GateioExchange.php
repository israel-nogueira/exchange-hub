...
{
    private GateioSigner $signer; private GateioNormalizer $normalizer;
    protected function configure(): void { $this->name='gateio'; $this->baseUrl=GateioConfig::BASE_URL; $this->signer=new GateioSigner($this->apiKey,$this->apiSecret); $this->normalizer=new GateioNormalizer(); }

    public function ping(): bool { try{$this->http->get($this->baseUrl.'/api',[],'gateio');return true;}catch(\Exception $e){return false;} }
    public function getServerTime(): int { return time()*1000; }
    public function getExchangeInfo(): ExchangeInfoDTO { return new ExchangeInfoDTO('Gateio','ONLINE',[],0.002,0.002,[],[],time()*1000); }
